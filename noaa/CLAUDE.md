...
- **Sources**: NOAA AIS broadcasts (CSV → DuckDB via Dagster asset) and Open-Meteo marine hourly
  (dlt pipeline — incremental by date cursor)
- **Warehouse**: DuckDB (`noaa.duckdb`)
- **Orchestration**: Dagster via `noaa_platform/`; run `dagster dev` from `noaa/`
